...
export default function HeroSection({ settings }: HeroSectionProps) {
  const headline = settings?.heroHeadline ?? 'Verify News Before You Believe It';
  const subheadline = settings?.heroSubheadline ?? 'Factify uses AI and trusted news sources to analyze articles, claims, and URLs, helping you identify misinformation and discover the truth behind every story.';
  const accuracy = settings?.statAccuracy ?? 95;
  const sources = settings?.statSources ?? 500;

  return (
    <section className="relative overflow-hidden hero-pattern pt-10 pb-16 sm:pt-16 sm:pb-24 lg:pt-24 lg:pb-32">
      <div className="wrapper">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-factify-navy leading-tight tracking-tight">
              {headline.includes('Believe') ? (
                <>
                  Verify News Before{' '}
            ...
            <p className="mt-4 sm:mt-6 text-base sm:text-lg text-factify-gray-dark leading-relaxed max-w-xl">{subheadline}</p>
            ...
            <div className="mt-10 flex flex-wrap gap-6 text-sm text-factify-gray-dark">
              {[
                { icon: CheckCircle2, text: `${accuracy}% Accuracy` },
                { icon: Globe, text: `${sources}+ Sources` },
          ...
          >
            <div className="relative rounded-2xl border border-factify-gray bg-white p-4 sm:p-6 shadow-factify-lg max-w-md mx-auto lg:max-w-none">
